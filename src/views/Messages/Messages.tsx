...
import {
    ...
    removeJobOffer,
    sendMessage,
} from 'feautures/messages/messagesSlice'
...
const SingleMessage = ({ text, sender }) => {
    ...
    return (
        <div className='single-message' style={style}>
    ...
    const handleAccept = async (id: number) => {
        ...
        if ((resp.meta.requestStatus = 'fulfilled')) {
            ...
            setCurrentChat((prev) => {
                ...
                console.log(copy)
                copy.jobOffers = copy.jobOffers.filter((jo) => jo.id !== id)
                return copy
            })
            setModalOpen(false)
            navigate('/tasks/' + currentChat.jobOffers.find((jo) => jo.id == id).job.id)
        }
    }
...
export const Messages = () => {
    ...
    const [params, setParams] = useState({ page: 1, limit: 20, conversationId })
    ...
    useEffect(() => {
        socket.on('message', (data) => {
            if (
                Number(conversationId) == data.conversationId &&
                !localMessages.some((m) => m == data.id)
            ) {
    ...
    useEffect(() => {
        currentChat && dispatch(getMessages(params))
    }, [params])

    useEffect(() => {
        setParams({ page: 1, limit: 20, conversationId })
        ...
        if (messages && conversationId) {
            setLocalMessages((prev) => {
                const newMessages = messages.filter(
                    (message) => !prev.some((prevMessage) => prevMessage.id === message.id),
                )
    ...
    const handleAcceptOffer = async () => {
        if (currentChat?.jobOffers.length > 1) {
            ...
            if ((resp.meta.requestStatus = 'fulfilled')) {
                ...
                navigate(
                    '/tasks/' +
                    currentChat.jobOffers.find((jo) => jo.id == currentChat?.jobOffers[0].id).job.id,
                )
            }
        }
    }
    ...
    return (
        <div className='page-content-static'>
                        ...
                        <>
                            ...
                            <div className='messages-chat__top'>
                                ...
                                <div className='flex gap1 h100 align-center'>
                                    {currentChat?.jobOffers?.length > 0 && (
                                        <div className='accept-offer-chat' onClick={handleAcceptOffer}>
                                            <FontAwesomeIcon icon={faCheckCircle as IconProp} />
                                            <p>Prihvati ponudu</p>
                                        </div>
                                    )}
                                    <FontAwesomeIcon
                                        icon={faWarning as IconProp}
                                        onClick={() => {
                            ...
                            <div className='messages-chat__list'>
                                <div className='messages-chat__list-scroll' ref={scrollRef}>
                                    {localMessages.map((m) => (
                                        <SingleMessage text={m.content} sender={m.senderId == user.id} />
                                    ))}
                                </div>
                            </div>
